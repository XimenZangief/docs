...
> **Note**
> 是 Laravel 的新手嗎？參考看看 [Laravel Bootcamp](https://bootcamp.laravel.com) 提供的框架實作旅程，讓我們引導你建立最初的 Laravel 應用程式。
...
在你建立第一個 Laravel 專案之前，你要先確保你的電腦已經安裝 PHP 跟 [ Composer ](https://getcomposer.org/) 。如果你是 macOS 的開發者， PHP 跟 Composer 都可以透過 [ Homebrew ](https://brew.sh/) 安裝。除此之外，我們也建議你 [ 安裝 Node 和 NPM ](https://nodejs.org/)
...
> **Note**  
> 如果你想要在開發 Laravel 應用程式時有個起頭，看看我們的 [入門套件](/docs/{{version}}/starter-kits)。 Laravel 入門套件會為你的 Laravel 應用程式提供前後端驗證的基本結構。
...
> **Note**  
> 已經是 Docker 專家？不用擔心！關於 Sail 的一切可以在 Laravel 內的 `docker-compose.yml` 檔案自行修改設定。 
...
> **Note**  
> 想繼續了解更多關於 Laravel Sail, 可以前往 [ Sail 完整說明文件 ](/docs/{{version}}/sail).
...
> **Note**  
> 安裝並啟動 WSL2 之後，你必須確保 Docker Desktop 已經 [ 設定為使用 WSL2後端 ](https://docs.docker.com/docker-for-windows/wsl/)。
...
Sail 安裝程式在你的本機完全建立 Sail 的應用程式容器可能會花費數分鐘的時間。
...
> **Note**  
> 關於更多 `.env` 和環境基本設定的資訊，請閱讀完整的 [ 設定文件 ](/docs/{{version}}/configuration#environment-configuration)。
